...
- 兼容 STL 算法（如 `std::sort`、`std::for_each`）、回调函数、异步逻辑等场景；Lambda 本质是匿名类并[[学习/编程语言/C++/重载运算符|重载了 `operator()`]]。
...
- 若 Lambda 有捕获，无法转换为函数指针（需用 `std::function` 封装——可对比 [[学习/编程语言/Python/可调用协议]] 的思路）。
...
- [[学习/编程语言/C++/重载运算符]] — lambda 是 operator() 的语法糖
