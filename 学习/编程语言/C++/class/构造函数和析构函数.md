...
意味着构造子类的时候[[继承|基类]]的构造函数和自身的都会调用
# 成员初始化列表
...
-  **继承场景**：子类初始化父类（见 [[继承]]，尤其是父类无默认构造时），必须在初始化列表中调用父类带参构造。
```cpp
#include <iostream>
using namespace std;
...
默认的拷贝构造函数会将class中的所有成员变量复制。如果有指针——这是[[智能指针|智能指针]]要解决的核心问题之一，那么拷贝的是指针而不是指向的内容，即两个指针指向同一块内存
