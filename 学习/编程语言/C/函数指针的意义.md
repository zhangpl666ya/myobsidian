...
tags:
  ...
  - "笔记/概念入门"
---
函数指针的核心意义在于**将函数“当作数据”来操作**——允许像传递变量、存储变量一样传递和存储函数，从而极大提升代码的灵活性、通用性和模块化程度。它是C语言中实现"行为抽象""动态逻辑"的关键工具（[[C语言中的面向对象编程]] 用它模拟 OOP；C++ 则用 [[C++/Lambda]] 进化了这个思路），在实际开发中有诸多重要应用：
...
- [[C语言中的面向对象编程]] — 函数指针是 C 模拟 OOP 的核心
- [[C++/Lambda]] — C++ 对函数指针的进化
- [[Python/可调用协议]] — 跨语言对应：可调用
